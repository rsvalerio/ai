...
- **Serial work inside a stream loop**: `while let Some(x) = s.next().await { work(x).await }` processes one item at a time no matter how many are ready. Fix: `futures::StreamExt`'s `buffer_unordered` (after a `.map(work)`) or `for_each_concurrent`, with an explicit concurrency limit (ASYNC-5, ASYNC-13)
