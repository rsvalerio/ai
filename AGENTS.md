...
- **rust-make-clippy-pedantic** — mechanical counterpart to `code-review-rust`: runs `cargo clippy` with the strict lint groups passed as flags (never as source edits) over a verified-clean tree, files one `Triage` task per warning labelled `pedantic` — dropping test-only style findings, generated files and out-of-tree warnings, and collapsing any lint that fires more than 20 times in one crate into a single aggregate task — and reports an effort estimate. Feeds `code-review-triage` like the review skills do.
