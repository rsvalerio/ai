...
| `Runtime::shutdown_timeout` as the blocking-work bound | 1s timeout against a 10s blocking task | Returned in 1.0005s — straggler thread abandoned on the chosen deadline; takes `self`, so needs an explicitly built runtime |
